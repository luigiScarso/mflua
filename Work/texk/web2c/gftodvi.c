...
#endif
{
  ...
  {
    readtfmword () ;
    lf = b0 * intcast ( 256 ) + b1 ;
    lh = b2 * intcast ( 256 ) + b3 ;
    readtfmword () ;
    bc = b0 * intcast ( 256 ) + b1 ;
    ec = b2 * intcast ( 256 ) + b3 ;
    if ( ( bc > ec + 1 ) || ( ec > 255 ) ) 
    goto lab11 ;
    if ( bc > 255 ) 
    {
    ...
    readtfmword () ;
    nw = b0 * intcast ( 256 ) + b1 ;
    nh = b2 * intcast ( 256 ) + b3 ;
    readtfmword () ;
    nd = b0 * intcast ( 256 ) + b1 ;
    ni = b2 * intcast ( 256 ) + b3 ;
    readtfmword () ;
    nl = b0 * intcast ( 256 ) + b1 ;
    nk = b2 * intcast ( 256 ) + b3 ;
    readtfmword () ;
    ne = b0 * intcast ( 256 ) + b1 ;
    np = b2 * intcast ( 256 ) + b3 ;
  ...
  parambase [f ]= extenbase [f ]+ ne ;
  {
    ...
    if ( b0 > 127 ) 
    goto lab11 ;
    z = ( ( b0 * intcast ( 256 ) + b1 ) * intcast ( 256 ) + b2 ) * 16 + ( b3 / 
    16 ) ;
    if ( z < 65536L ) 
    goto lab11 ;
    while ( lh > 2 ) {
  ...
  } 
  {
      ...
      f ]- 1 ; if ( k <= for_end) do 
	{
	  ...
	  if ( b0 > 128 ) 
	  {
	    ...
	    else if ( intcast ( 256 ) * ( b2 - 128 ) + b3 >= nk ) 
	    goto lab11 ;
	  } 
	} 
      while ( k++ < for_end ) ;} 
