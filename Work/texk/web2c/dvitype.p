...
{:4}const{5:}maxfonts=500;maxwidths=25000;linelength=79;
...
{:122}{112:}procedure parsearguments;const noptions=8;
...
begin outmode:=optarg[0]-'0';
if(outmode<0)or(outmode>4)or(optarg[1]<>0)then begin writeln(stderr,
'Value for --output-level must be >= 0 and <= 4.');uexit(1);end;
...
'dvitype: Need exactly one file argument.');usage('dvitype');end;end;
...
read(dvifile,b);curloc:=curloc+2;gettwobytes:=a*intcast(256)+b;end;
...
{:27}{28:}function dvilength:integer;
begin xfseek(dvifile,0,2,'dvitype');curloc:=xftell(dvifile,'dvitype');
dvilength:=curloc;end;procedure movetobyte(n:integer);
begin xfseek(dvifile,n,0,'dvitype');curloc:=n;end;
...
begin{35:}readtfmword;lh:=b2*intcast(256)+b3;readtfmword;
fontbc[nf]:=b0*intcast(256)+b1;fontec[nf]:=b2*intcast(256)+b3;
...
if k=4 then if b0<128 then tfmchecksum:=((b0*intcast(256)+b1)*256+b2)
*256+b3 else tfmchecksum:=(((b0-256)*intcast(256)+b1)*256+b2)*256+b3
else if k=5 then if b0<128 then tfmdesignsize:=round(tfmconv*(((b0*256+
b1)*256+b2)*256+b3))else goto 9997;end;{:35};
