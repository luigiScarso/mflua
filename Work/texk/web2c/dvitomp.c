...
#define maxfnums ( 3000 ) 
#define maxwidths ( 256 * maxfonts ) 
#define virtualspace ( 1000000L ) 
#define linelength ( 79 ) 
#define stacksize ( 100 ) 
#define namesize ( 100000L ) 
#define namelength ( 250 ) 
#define maxnamedcolors ( 100 ) 
#define maxcolorstackdepth ( 10 ) 
...
namedcolorrecord namedcolors[maxnamedcolors + 1]  ;
integer numnamedcolors  ;
integer colorstackdepth  ;
cstring colorstack[maxcolorstackdepth + 1]  ;
...
#define noptions ( 2 ) 
  ...
  do {
    ...
    ) ) 
    {
      ...
      "AT&T Bell Laboraties" , "John Hobby." , nil ) ;
    } 
...
void 
#ifdef HAVE_PROTOTYPES
zdefnamedcolor ( cstring n , cstring v ) 
#else
zdefnamedcolor ( n , v ) 
  cstring n ;
  cstring v ;
#endif
{
  if ( numnamedcolors == maxnamedcolors ) 
  {
    fprintf (stdout, "%s%s\n", "DVItoMP abort: " , "too many named color definitions" ) ;
    history = 3 ;
    uexit ( history ) ;
  } 
  else if ( ( numnamedcolors > 0 ) && ( strcmp ( n , namedcolors [
  numnamedcolors ].name ) <= 0 ) ) 
  {
    fprintf (stdout, "%s%s\n", "DVItoMP abort: " ,     "named colors added out of alphabetical order" ) ;
    history = 3 ;
    uexit ( history ) ;
  } 
  numnamedcolors = numnamedcolors + 1 ;
  namedcolors [numnamedcolors ].name = n ;
  namedcolors [numnamedcolors ].value = v ;
} 
void 
#ifdef HAVE_PROTOTYPES
initialize ( void ) 
...
#endif
{
  ...
  if ( a < 128 ) 
  Result = ( a * intcast ( 256 ) + b ) * 256 + c ;
  else Result = ( ( a - intcast ( 256 ) ) * 256 + b ) * 256 + c ;
  return Result ;
} 
integer 
#ifdef HAVE_PROTOTYPES
...
#endif
{
  ...
  if ( a < 128 ) 
  Result = ( ( a * intcast ( 256 ) + b ) * 256 + c ) * 256 + d ;
  else Result = ( ( ( a - 256 ) * intcast ( 256 ) + b ) * 256 + c ) * 256 + d 
  ;
  return Result ;
} 
void 
  ...
  {register integer for_end; k = 1 ;for_end = 3 + lh ; if ( k <= for_end) do 
    {
      ...
      {
	if ( b0 < 128 ) 
	tfmchecksum = ( ( b0 * intcast ( 256 ) + b1 ) * 256 + b2 ) * 256 + b3 
	;
	else tfmchecksum = ( ( ( b0 - 256 ) * intcast ( 256 ) + b1 ) * 256 + 
	b2 ) * 256 + b3 ;
      } 
    } 
  while ( k++ < for_end ) ;} 
...
#endif
{
  register integer Result; Result = 0 ;
  switch ( o ) 
  {case 0 : 
  case 1 : 
  case 2 : 
...
#endif
{
  ...
  if ( infobase [f ]== maxwidths ) 
  {
    ...
    if ( ff < nf ) 
    {
      ...
      curname = xmallocarray ( char , fontname [f + 1 ]- fontname [f ]) ;
      {register integer for_end; k = fontname [f ];for_end = fontname [f 
      + 1 ]- 1 ; if ( k <= for_end) do 
	{
...
#endif
{
  ...
  if ( strf >= 0 ) 
  {
    ...
    {
      Fputs ( mpxfile ,  " withcolor " ) ;
      fputs ( colorstack [colorstackdepth ], mpxfile ) ;
    } 
    fprintf ( mpxfile , "%s\n",  ");" ) ;
    strf = -1 ;
  } 
...
#endif
{
  ...
  printchar ( c ) ;
  strh2 = h + floor ( dviscale * fontscaledsize [f ]* width [infobase [f ]
  + c ]) ;
} 
void 
#ifdef HAVE_PROTOTYPES
...
#define bufsiz ( 256 ) 
  ...
  if ( ( len <= 5 ) || ( buf [0 ]!= 99 ) || ( buf [1 ]!= 111 ) || ( buf [
  2 ]!= 108 ) || ( buf [3 ]!= 111 ) || ( buf [4 ]!= 114 ) || ( buf [5 ]
  != 32 ) ) 
  goto lab9999 ;
  if ( p > 0 ) 
  {
    {
      fprintf (stdout, "%s%s\n", "DVItoMP warning: " , "long \"color\" special ignored" ) ;
      if ( history < 2 ) 
      history = 1 ;
    } 
    goto lab9999 ;
  } 
  if ( ( len == 9 ) && ( buf [6 ]== 112 ) && ( buf [7 ]== 111 ) && ( buf [
  8 ]== 112 ) ) 
  {
    finishlastchar () ;
    if ( colorstackdepth > 0 ) 
    {
      free ( colorstack [colorstackdepth ]) ;
  ...
  else if ( ( len >= 11 ) && ( buf [6 ]== 112 ) && ( buf [7 ]== 117 ) && ( 
  buf [8 ]== 115 ) && ( buf [9 ]== 104 ) && ( buf [10 ]== 32 ) ) 
  {
    finishlastchar () ;
    if ( colorstackdepth >= maxcolorstackdepth ) 
    {
      fprintf (stdout, "%s%s\n", "DVItoMP abort: " , "color stack overflow" ) ;
      history = 3 ;
      uexit ( history ) ;
    } 
    colorstackdepth = colorstackdepth + 1 ;
    l = 11 ;
    while ( ( l < len - 1 ) && ( buf [l ]== 32 ) ) l = l + 1 ;
    if ( ( l + 4 < len ) && ( buf [l ]== 114 ) && ( buf [l + 1 ]== 103 ) 
    && ( buf [l + 2 ]== 98 ) && ( buf [l + 3 ]== 32 ) ) 
    {
      l = l + 4 ;
      while ( ( l < len ) && ( buf [l ]== 32 ) ) l = l + 1 ;
      while ( ( len > l ) && ( buf [len - 1 ]== 32 ) ) len = len - 1 ;
      colorstack [colorstackdepth ]= xmallocarray ( char , len - l + 3 ) ;
      k = 0 ;
      colorstack [colorstackdepth ][k ]= 40 ;
      k = k + 1 ;
      while ( l < len ) if ( buf [l ]== 32 ) 
      {
	colorstack [colorstackdepth ][k ]= 44 ;
	while ( ( l < len ) && ( buf [l ]== 32 ) ) l = l + 1 ;
	k = k + 1 ;
      } 
      else {
	  
	colorstack [colorstackdepth ][k ]= buf [l ];
	l = l + 1 ;
	k = k + 1 ;
      } 
      colorstack [colorstackdepth ][k ]= 41 ;
      colorstack [colorstackdepth ][k + 1 ]= chr ( 0 ) ;
    } 
    else if ( ( l + 5 < len ) && ( buf [l ]== 99 ) && ( buf [l + 1 ]== 109 
    ) && ( buf [l + 2 ]== 121 ) && ( buf [l + 3 ]== 107 ) && ( buf [l + 4 
    ]== 32 ) ) 
    {
      l = l + 5 ;
      while ( ( l < len ) && ( buf [l ]== 32 ) ) l = l + 1 ;
      ...
      strcpy ( colorstack [colorstackdepth ], "cmyk" ) ;
      k = 4 ;
      colorstack [colorstackdepth ][k ]= 40 ;
      k = k + 1 ;
      while ( l < len ) if ( buf [l ]== 32 ) 
    ...
    else if ( ( l + 5 < len ) && ( buf [l ]== 103 ) && ( buf [l + 1 ]== 
    114 ) && ( buf [l + 2 ]== 97 ) && ( buf [l + 3 ]== 121 ) && ( buf [l 
    + 4 ]== 32 ) ) 
    {
      l = l + 5 ;
      while ( ( l < len ) && ( buf [l ]== 32 ) ) l = l + 1 ;
      ...
      strcpy ( colorstack [colorstackdepth ], "white*" ) ;
      k = 6 ;
      colorstack [colorstackdepth ][k ]= 40 ;
      k = k + 1 ;
      while ( l < len ) if ( buf [l ]== 32 ) 
	...
	k = k + 1 ;
      ...
      {register integer for_end; k = l ;for_end = len - 1 ; if ( k <= 
      for_end) do 
	{
	  buf [k - l ]= xchr [buf [k ]];
	} 
	...
	if ( k == 0 ) 
	{
	  colorstack [colorstackdepth ]= xstrdup ( namedcolors [m ].value 
	  ) ;
	  found = true ;
	} 
	else if ( k < 0 ) 
	r = m - 1 ;
      ...
      {
	{
	  fprintf (stdout, "%s%s%s%s\n", "DVItoMP warning: " , "non-hardcoded color \"" ,           stringcast ( buf ) , "\" in \"color push\" command" ) ;
	  if ( history < 2 ) 
	  history = 1 ;
	} 
	colorstack [colorstackdepth ]= xstrdup ( stringcast ( buf ) ) ;
      } 
    } 
  } 
  else {
    ...
    {
      fprintf (stdout, "%s%s\n", "DVItoMP warning: " , "unknown \"color\" special ignored" ) ;
      if ( history < 2 ) 
      history = 1 ;
    } 
    goto lab9999 ;
  ...
  lab9999: {
      register integer for_end; k = 1 ;for_end = p ; if ( k <= 
  for_end) do 
    downthedrain = getbyte () ;
  while ( k++ < for_end ) ;} 
} 
...
#endif
{
  ...
  while ( true ) {
	...
	if ( vfreading ) 
	{
      ...
      h = h + floor ( dviscale * fontscaledsize [curfont ]* width [infobase 
      [curfont ]+ p ]) ;
    } 
    else switch ( o ) 
    {case 133 : 
...
void mainbody() {
  ...
  openmpxfile () ;
  Fputs ( mpxfile ,  "% Written by DVItoMP, Version 1.003/color" ) ;
  fprintf ( mpxfile , "%s\n",  versionstring ) ;
  {
    while ( true ) {
