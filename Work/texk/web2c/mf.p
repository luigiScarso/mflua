...
{:6}const{11:}maxinternal=300;screenwidth=1664;screendepth=1200;
stacksize=300;maxstrings=7500;stringvacancies=74000;poolsize=100000;
movesize=20000;maxwiggle=1000;poolname=TEXMFPOOLNAME;
enginename=TEXMFENGINENAME;pathsize=1000;bistacksize=1500;
headersize=100;ligtablesize=15000;maxkerns=2500;maxfontdimen=60;
infmainmemory=3000;supmainmemory=8000000;infbufsize=500;
supbufsize=300000;{:11}type{18:}ASCIIcode=0..255;
...
memtop:integer;memmax:integer;bufsize:integer;errorline:integer;
halferrorline:integer;maxprintline:integer;gfbufsize:integer;
...
{:768}{775:}basedefaultlength:integer;MFbasedefault:^char;
...
{:43}{:73}{76:}procedure jumpout;begin closefilesandterminate;
...
if r>intcast(p+1)then{170:}begin mem[p].hh.lh:=r-p;rover:=p;goto 40;
end{:170};
...
begin mfluaprintpath(h,s,nuline);printdiagnostic(517,s,nuline);println;
...
t:=areadelimiter+1;j:=s;
while(not mustquote)and(j<>t)do begin mustquote:=strpool[j]=' ';incr(j);
...
areadelimiter:=areadelimiter+2;poolptr:=poolptr+2;end;
s:=areadelimiter+1;end else begin s:=strstart[strptr];end;
if extdelimiter=0 then t:=poolptr else t:=extdelimiter;mustquote:=false;
...
end;if extdelimiter<>0 then begin s:=extdelimiter;t:=poolptr;
mustquote:=false;j:=s;
while(not mustquote)and(j<>t)do begin mustquote:=strpool[j]=' ';incr(j);
end;if mustquote then begin strpool[t+1]:=34;
for j:=t-1 downto s do strpool[j+1]:=strpool[j];strpool[s]:=34;
poolptr:=poolptr+2;end;end;
if areadelimiter=0 then curarea:=261 else begin curarea:=strptr;
incr(strptr);strstart[strptr]:=areadelimiter+1;end;
if extdelimiter=0 then begin curext:=261;curname:=makestring;
...
{790:}begin write(logfile,'This is METAFONT, Version 2.718281');
...
var tempstr:strnumber;k:integer;
...
packfilename(curname,curarea,curext);end;
if openinnameok(stringcast(nameoffile+1))and aopenin(inputfile[curinput.
...
if(intcast(mem[curedges+2].hh.lh)+tx<=0)or(mem[curedges+2].hh.rh+tx>=
8192)or(intcast(mem[curedges+1].hh.lh)+ty<=0)or(mem[curedges+1].hh.rh+ty
>=8191)or(abs(tx)>=4096)or(abs(ty)>=4096)then begin begin if interaction
=3 then;
if(filelineerrorstylep and not(curinput.namefield=0))then begin printnl(
261);print(fullsourcefilenamestack[inopen]);print(58);printint(line);
...
if prevm-intcast(mem[curedges+3].hh.lh)+xoff>gfmaxm then gfmaxm:=prevm-
mem[curedges+3].hh.lh+xoff{:1169};p:=mem[p].hh.lh;decr(n);end;
...
if internal[11]>0 then printedges(1056,true,xoff,yoff);end;
...
baseengine:^ASCIIcode;begin{1200:}selector:=5;print(1072);
...
strcpy(baseengine,enginename);for k:=x to x+3 do baseengine[k]:=0;
x:=x+4-(x mod 4);dumpint(x);dumpthings(baseengine[0],x);
libcfree(baseengine);dumpint(523964609);{1216:}dumpthings(xord[0],256);
dumpthings(xchr[0],256);dumpthings(xprn[0],256);{:1216};dumpint(0);
dumpint(memtop);dumpint(9500);dumpint(7919);dumpint(15){:1190};
{1192:}dumpint(poolptr);dumpint(strptr);
...
w.b3:=strpool[k+3];dumpqqqq(w);println;printint(strptr);print(1069);
...
dumpint(varused);dumpint(dynused);println;printint(x);print(1070);
...
dumpint(stcount);println;printint(stcount);print(1071){:1196};
...
if strcmp(enginename,baseengine)then begin;
writeln(stdout,'---! ',stringcast(nameoffile+1),' was written by ',
baseengine);libcfree(baseengine);goto 6666;end;libcfree(baseengine);
undumpint(x);if x<>523964609 then begin;
...
if totalchars<>1 then print(1064)else print(1065);print(1066);
...
while openparens>0 do begin print(1076);decr(openparens);end;
...
goto 10;end;endif('INIMF')printnl(1081);goto 10;end;10:end;
...
{:55}{61:}write(stdout,'This is METAFONT, Version 2.718281');
write(stdout,versionstring);if baseident>0 then slowprint(baseident);
println;if translatefilename then begin write(stdout,'(');
