...
{:181}procedure closefilesandterminate;forward;procedure jumpout;
...
{:24}{25:}procedure overflow(p:pcktpointer;n:int16ut);
begin write(termout,' !Sorry, ','DVIcopy',' capacity exceeded [');
printpacket(p);writeln(termout,'=',n:1,'].');jumpout;end;
{:25}{94:}procedure badtfm;begin write(termout,'Bad TFM file');
...
'dvicopy: Need at most two file arguments.');usage('dvicopy');end;end;
...
))*intcast(256)+b;end;function pcktupair:int16ut;var a,b:eightbits;
begin{if curloc>=curlimit then confusion(strpackets)else}
begin a:=bytemem[curloc];curloc:=curloc+1;end;
{if curloc>=curlimit then confusion(strpackets)else}
begin b:=bytemem[curloc];curloc:=curloc+1;end;
pcktupair:=a*intcast(256)+b;end;{:51}{52:}function pcktstrio:int24t;
var a,b,c:eightbits;
begin{if curloc>=curlimit then confusion(strpackets)else}
begin a:=bytemem[curloc];curloc:=curloc+1;end;
{if curloc>=curlimit then confusion(strpackets)else}
...
function pcktutrio:int24ut;var a,b,c:eightbits;
begin{if curloc>=curlimit then confusion(strpackets)else}
begin a:=bytemem[curloc];curloc:=curloc+1;end;
{if curloc>=curlimit then confusion(strpackets)else}
begin b:=bytemem[curloc];curloc:=curloc+1;end;
...
n:pcktpointer;curloc,curlimit:bytepointer;{ll:int15t;}c:char;
...
f:eightbits;e:int24t;begin{87:}q:=charpackets[fntchars[curfnt]+curres];
...
curlimit:bytepointer;begin{87:}q:=charpackets[fntchars[curfnt]+curres];
while q<>maxpackets do begin p:=q;q:=maxpackets;curloc:=pcktstart[p];
curlimit:=pcktstart[p+1];if p=0 then begin e:=0;f:=0;
end else begin{if curloc>=curlimit then confusion(strpackets)else}
begin f:=bytemem[curloc];curloc:=curloc+1;end;case(f div 64)of 0:e:=0;
...
tfmb2)*intcast(256)+tfmb3)));end;end{:101};{102:}readtfmword;
...
begin xfseek(dvifile,0,2,'dvicopy');dviloc:=xftell(dvifile,'dvicopy');
dvilength:=dviloc;end;procedure dvimove(n:integer);
begin xfseek(dvifile,n,0,'dvicopy');dviloc:=n;end;
...
*intcast(256)+b;end;function dviupair:int16ut;var a,b:eightbits;
begin if eof(dvifile)then baddvi else read(dvifile,a);
if eof(dvifile)then baddvi else read(dvifile,b);dviloc:=dviloc+2;
dviupair:=a*intcast(256)+b;end;function dvistrio:int24t;
var a,b,c:eightbits;
begin if eof(dvifile)then baddvi else read(dvifile,a);
...
function dviutrio:int24ut;var a,b,c:eightbits;
begin if eof(dvifile)then baddvi else read(dvifile,a);
if eof(dvifile)then baddvi else read(dvifile,b);
if eof(dvifile)then baddvi else read(dvifile,c);dviloc:=dviloc+3;
...
intcast(256)+d;end;{:113}{114:}function dviuquad:int31t;var x:integer;
...
vfupair:=a*intcast(256)+b;end;function vfstrio:int24t;
var a,b,c:eightbits;
begin if eof(vffile)then badfont else read(vffile,a);
...
(a-intcast(256))*intcast(256)+b)*intcast(256)+c;end;
function vfutrio:int24ut;var a,b,c:eightbits;
begin if eof(vffile)then badfont else read(vffile,a);
if eof(vffile)then badfont else read(vffile,b);
if eof(vffile)then badfont else read(vffile,c);vfloc:=vfloc+3;
...
intcast(256)+d;end;{:141}{143:}function vffix1:integer;var x:integer;
...
function vffixp:int31t;var x:integer;
begin if eof(vffile)then badfont else read(vffile,tfmb0);
if eof(vffile)then badfont else read(vffile,tfmb1);
if eof(vffile)then badfont else read(vffile,tfmb2);
if eof(vffile)then badfont else read(vffile,tfmb3);vfloc:=vfloc+4;
...
{:145}{148:}procedure vffont;var f:fontnumber;begin{149:}f:=0;
vfefnts[vfnf]:=curparm;while curparm<>vfefnts[f]do f:=f+1{:149};
if f=vfnf then badfont;vfcurfnt:=vfifnts[f];end;
{:148}{150:}procedure vfdofont;var f:fontnumber;k:int15t;
...
label 21,30,32,10;var tempint:integer;tempbyte:int8ut;k:bytepointer;
l:int15t;saveext:int24t;saveres:int8ut;savecp:widthpointer;
...
if true then{232:}begin dvistart:=dviloc;{233:}tempint:=dvilength-5;
...
17:goto 30;others:;end;dvifirstpar;end{:237};
