...
#endif
{
  ...
  ;
  cursize = bufsize ;
  BIBXRETALLOC ( "sv_buffer" , svbuffer , ASCIIcode , cursize , cursize + 
  BUFSIZE ) ;
  cursize = bufsize ;
  ...
  cursize = bufsize ;
  BIBXRETALLOC ( "name_tok" , nametok , bufpointer , cursize , cursize + 
  BUFSIZE ) ;
  cursize = bufsize ;
...
#endif
{
  ...
  while ( ( outbuflength > maxprintline ) ) {
    ...
    {
      if ( outbuf [maxprintline - 1 ]== 37 ) 
      outbuflength = maxprintline - 1 ;
      else outbuflength = maxprintline ;
      breakptr = outbuflength - 1 ;
      outbuf [endptr ]= outbuf [breakptr ];
      outbuf [breakptr ]= 37 ;
      outputbblline () ;
      outbuf [breakptr ]= outbuf [endptr ];
      outbufptr = 0 ;
      tmpptr = breakptr ;
      while ( ( tmpptr < endptr ) ) {
