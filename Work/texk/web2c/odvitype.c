...
#endif
{
  ...
  read ( dvifile , b ) ;
  curloc = curloc + 2 ;
  Result = intcast ( a ) * 256 + intcast ( b ) ;
  return Result ;
} 
integer 
#ifdef HAVE_PROTOTYPES
...
#endif
{
  ...
  read ( dvifile , c ) ;
  curloc = curloc + 3 ;
  Result = ( intcast ( a ) * 256 + intcast ( b ) ) * 256 + intcast ( c ) ;
  return Result ;
} 
integer 
#ifdef HAVE_PROTOTYPES
...
#endif
{
  ...
  curloc = curloc + 3 ;
  if ( a < 128 ) 
  Result = ( intcast ( a ) * 256 + b ) * 256 + intcast ( c ) ;
  else Result = ( ( intcast ( a ) - 256 ) * 256 + intcast ( b ) ) * 256 + 
  intcast ( c ) ;
  return Result ;
} 
integer 
...
#endif
{
  ...
  curloc = curloc + 4 ;
  if ( a < 128 ) 
  Result = ( ( intcast ( a ) * 256 + b ) * 256 + intcast ( c ) ) * 256 + 
  intcast ( d ) ;
  else Result = ( ( ( intcast ( a ) - 256 ) * 256 + intcast ( b ) ) * 256 + 
  intcast ( c ) ) * 256 + intcast ( d ) ;
  return Result ;
} 
integer 
#ifdef HAVE_PROTOTYPES
...
#endif
{
  register integer Result; xfseek ( dvifile , 0 , 2 , "odvitype" ) ;
  curloc = xftell ( dvifile , "odvitype" ) ;
  Result = curloc ;
  return Result ;
} 
void 
...
#endif
{
  ...
  integer j, kprime  ;
  readtfmword () ;
  if ( ( intcast ( b0 ) * 256 + intcast ( b1 ) ) != 0 ) 
  {
    ofmlevel = -1 ;
	...
	if ( k == 4 ) 
	if ( b0 < 128 ) 
	tfmchecksum = ( ( intcast ( b0 ) * 256 + intcast ( b1 ) ) * 256 + 
	intcast ( b2 ) ) * 256 + intcast ( b3 ) ;
	else tfmchecksum = ( ( ( intcast ( b0 ) - 256 ) * 256 + intcast ( b1 ) 
	) * 256 + intcast ( b2 ) ) * 256 + intcast ( b3 ) ;
	else if ( k == 5 ) 
	if ( b0 < 128 ) 
    ...
    ofmlevel = intcast ( b2 ) * 256 + intcast ( b3 ) ;
    readtfmword () ;
    readtfmword () ;
    lh = ( ( intcast ( b0 ) * 256 + intcast ( b1 ) ) * 256 + intcast ( b2 ) ) 
    * 256 + intcast ( b3 ) ;
    readtfmword () ;
    fontbc [nf ]= ( ( intcast ( b0 ) * 256 + intcast ( b1 ) ) * 256 + 
    intcast ( b2 ) ) * 256 + intcast ( b3 ) ;
    readtfmword () ;
    fontec [nf ]= ( ( intcast ( b0 ) * 256 + intcast ( b1 ) ) * 256 + 
    intcast ( b2 ) ) * 256 + intcast ( b3 ) ;
    if ( fontec [nf ]< fontbc [nf ]) 
    fontbc [nf ]= fontec [nf ]+ 1 ;
    if ( widthptr + fontec [nf ]- fontbc [nf ]+ 1 > maxwidths ) 
    ...
    wp = widthptr + fontec [nf ]- fontbc [nf ]+ 1 ;
    readtfmword () ;
    nw = ( ( intcast ( b0 ) * 256 + intcast ( b1 ) ) * 256 + intcast ( b2 ) ) 
    * 256 + intcast ( b3 ) ;
    if ( ( nw == 0 ) || ( nw > 65536L ) ) 
    goto lab9997 ;
    ...
    {
      readtfmword () ;
      nco = ( ( ( b0 * 256 + b1 ) * 256 + b2 ) * 256 + b3 ) / 2 ;
      readtfmword () ;
      readtfmword () ;
      extrawords = ( ( ( intcast ( b0 ) * 256 + intcast ( b1 ) ) * 256 + 
      intcast ( b2 ) ) * 256 + intcast ( b3 ) ) / 2 ;
  ...
  case 0 : 
	...
	{
	  readtfmword () ;
	  if ( ( intcast ( b0 ) * 256 + intcast ( b1 ) ) > nw ) 
	  goto lab9997 ;
