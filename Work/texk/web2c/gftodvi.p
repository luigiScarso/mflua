...
'gftodvi: Need exactly one file argument.');usage('gftodvi');end;end;
...
gettwobytes:=a*intcast(256)+b;end;function getthreebytes:integer;
var a,b,c:eightbits;begin read(gffile,a);read(gffile,b);read(gffile,c);
curloc:=curloc+3;getthreebytes:=(a*intcast(256)+b)*256+c;end;
...
alpha:integer;beta:1..16;begin{59:}{60:}begin readtfmword;
lf:=b0*intcast(256)+b1;lh:=b2*intcast(256)+b3;readtfmword;
bc:=b0*intcast(256)+b1;ec:=b2*intcast(256)+b3;
if(bc>ec+1)or(ec>255)then goto 11;if bc>255 then begin bc:=1;ec:=0;end;
readtfmword;nw:=b0*intcast(256)+b1;nh:=b2*intcast(256)+b3;readtfmword;
nd:=b0*intcast(256)+b1;ni:=b2*intcast(256)+b3;readtfmword;
nl:=b0*intcast(256)+b1;nk:=b2*intcast(256)+b3;readtfmword;
ne:=b0*intcast(256)+b1;np:=b2*intcast(256)+b3;
...
if b2<128 then begin if(b3<bc)or(b3>ec)then goto 11 end else if intcast(
...
{:92}{94:}procedure startgf;label 30;var argbuffer:^char;
...
packfilename(jobname,0,20);opendvifile;end;
{:94}{107:}procedure writedvi(a,b:dviindex);
begin writechunk(dvifile,dvibuf,a,b);end;{:107}{108:}procedure dviswap;
