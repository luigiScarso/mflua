...
{:4}const{5:}maxfonts=500;maxwidths=100000;linelength=79;
terminallinelength=150;stacksize=100;namesize=10000;
{:5}type{8:}ASCIIcode=0..255;{:8}{9:}textfile=packed file of ASCIIcode;
...
count:array[0..9]of integer;{:42}{58:}inpostamble:boolean;
{:58}{68:}textptr:0..linelength;
textbuf:array[1..linelength]of ASCIIcode;
{:68}{73:}h,v,w,x,y,z,hh,vv:integer;
...
curfont:integer;showing:boolean;{:79}{98:}oldbackpointer:integer;
newbackpointer:integer;started:boolean;{:98}{102:}postloc:integer;
firstbackpointer:integer;startloc:integer;afterpre:integer;
{:102}{109:}k,m,n,p,q:integer;{:109}{123:}showopcodes:cinttype;
{:123}{113:}procedure parsearguments;const noptions=8;
var longoptions:array[0..noptions]of getoptstruct;
getoptreturnval:integer;optionindex:cinttype;currentoption:0..noptions;
endnum:^char;begin{114:}currentoption:=0;
...
{:114}{115:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:115}{116:}longoptions[currentoption].name:='output-level';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
outmode:=4;{:116}{117:}longoptions[currentoption].name:='page-start';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:117}{119:}longoptions[currentoption].name:='max-pages';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
maxpages:=1000000;{:119}{120:}longoptions[currentoption].name:='dpi';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
resolution:=300.0;
{:120}{121:}longoptions[currentoption].name:='magnification';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
newmag:=0;{:121}{122:}longoptions[currentoption].name:='show-opcodes';
...
newmag:=0;{:122}{124:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:124};
...
'odvitype: More than ten count registers specified.');uexit(1);end;
m:=m+1;
end else begin startcount[k]:=strtol(optarg+m,addressof(endnum),10);
if endnum=optarg+m then begin writeln(stderr,
'odvitype: -page-start values must be numeric or *.');uexit(1);end;
startthere[k]:=true;m:=m+endnum-(optarg+m);end;end;startvals:=k;{:118};
...
'odvitype: Need exactly one file argument.');usage('odvitype');end;end;
{:113}procedure initialize;var i:integer;begin kpsesetprogname(argv[0]);
parsearguments;write(stdout,'This is ODVItype, Version 1.5');
...
fontec[maxfonts]:=0;{:31}{43:}{:43}{59:}inpostamble:=false;
{:59}{69:}textptr:=0;{:69}{75:}maxv:=2147483548;maxh:=2147483548;
maxs:=stacksize+1;maxvsofar:=0;maxhsofar:=0;maxssofar:=0;pagecount:=0;
{:75}{99:}oldbackpointer:=-1;started:=false;{:99}end;
{:3}{23:}procedure opendvifile;
...
read(dvifile,b);curloc:=curloc+2;gettwobytes:=intcast(a)*256+intcast(b);
end;function signedpair:integer;var a,b:eightbits;begin read(dvifile,a);
...
getthreebytes:=(intcast(a)*256+intcast(b))*256+intcast(c);end;
function signedtrio:integer;var a,b,c:eightbits;begin read(dvifile,a);
read(dvifile,b);read(dvifile,c);curloc:=curloc+3;
if a<128 then signedtrio:=(intcast(a)*256+b)*256+intcast(c)else
signedtrio:=((intcast(a)-256)*256+intcast(b))*256+intcast(c);end;
function signedquad:integer;var a,b,c,d:eightbits;begin read(dvifile,a);
read(dvifile,b);read(dvifile,c);read(dvifile,d);curloc:=curloc+4;
if a<128 then signedquad:=((intcast(a)*256+b)*256+intcast(c))*256+
intcast(d)else signedquad:=(((intcast(a)-256)*256+intcast(b))*256+
intcast(c))*256+intcast(d);end;{:27}{28:}function dvilength:integer;
begin xfseek(dvifile,0,2,'odvitype');curloc:=xftell(dvifile,'odvitype');
dvilength:=curloc;end;procedure movetobyte(n:integer);
begin xfseek(dvifile,n,0,'odvitype');curloc:=n;end;
...
begin{35:}readtfmword;
if(intcast(b0)*256+intcast(b1))<>0 then begin ofmlevel:=-1;
lh:=intcast(b2)*256+intcast(b3);readtfmword;
fontbc[nf]:=intcast(b0)*256+intcast(b1);
fontec[nf]:=intcast(b2)*256+intcast(b3);
...
nw:=intcast(b0)*256+intcast(b1);if(nw=0)or(nw>256)then goto 9997;
for k:=1 to 3+lh do begin if eof(tfmfile)then goto 9997;readtfmword;
if k=4 then if b0<128 then tfmchecksum:=((intcast(b0)*256+intcast(b1))
*256+intcast(b2))*256+intcast(b3)else tfmchecksum:=(((intcast(b0)-256)
*256+intcast(b1))*256+intcast(b2))*256+intcast(b3)else if k=5 then if b0
<128 then tfmdesignsize:=round(tfmconv*(((b0*256+b1)*256+b2)*256+b3))
else goto 9997;end;end else begin ofmlevel:=intcast(b2)*256+intcast(b3);
readtfmword;readtfmword;
lh:=((intcast(b0)*256+intcast(b1))*256+intcast(b2))*256+intcast(b3);
readtfmword;
fontbc[nf]:=((intcast(b0)*256+intcast(b1))*256+intcast(b2))*256+intcast(
b3);readtfmword;
fontec[nf]:=((intcast(b0)*256+intcast(b1))*256+intcast(b2))*256+intcast(
b3);if fontec[nf]<fontbc[nf]then fontbc[nf]:=fontec[nf]+1;
if widthptr+fontec[nf]-fontbc[nf]+1>maxwidths then begin writeln(stdout,
'---not loaded, ODVItype needs larger width table');goto 9998;end;
wp:=widthptr+fontec[nf]-fontbc[nf]+1;readtfmword;
nw:=((intcast(b0)*256+intcast(b1))*256+intcast(b2))*256+intcast(b3);
...
nco:=(((b0*256+b1)*256+b2)*256+b3)div 2;readtfmword;readtfmword;
extrawords:=(((intcast(b0)*256+intcast(b1))*256+intcast(b2))*256+intcast
(b3))div 2;for k:=1 to 12 do begin if eof(tfmfile)then goto 9997;
readtfmword;end;end;
for k:=1 to lh do begin if eof(tfmfile)then goto 9997;readtfmword;
if k=1 then if b0<128 then tfmchecksum:=((intcast(b0)*256+intcast(b1))
*256+intcast(b2))*256+intcast(b3)else tfmchecksum:=(((intcast(b0)-256)
*256+intcast(b1))*256+intcast(b2))*256+intcast(b3)else if k=2 then if b0
<128 then tfmdesignsize:=round(tfmconv*(((b0*256+b1)*256+b2)*256+b3))
else goto 9997;end;
if ofmlevel=1 then for k:=1 to(nco-29-lh)do readtfmword;end{:35};
...
width[k]:=(intcast(b0)*256+intcast(b1));readtfmword;end;end;
1:begin if wp>0 then begin k:=widthptr;while k<wp do begin readtfmword;
if(intcast(b0)*256+intcast(b1))>nw then goto 9997;
width[k]:=(intcast(b0)*256+intcast(b1));readtfmword;readtfmword;
kprime:=k+(intcast(b0)*256+intcast(b1));
for j:=k+1 to kprime do width[j]:=width[k];k:=kprime;
...
9998:inTFM:=false;9999:end;{:34}{45:}function startmatch:boolean;
...
{:45}{60:}procedure definefont(e:integer);var f:0..maxfonts;p:integer;
...
if not showing then if m<>1000 then write(stdout,' scaled ',m:1){:62};
...
if f=nf then{63:}begin{67:}r:=fontname[nf+1]-fontname[nf];
curname:=xmallocarray(char,r);
for k:=fontname[nf]to fontname[nf+1]do begin curname[k-fontname[nf]]:=
xchr[names[k]];end;curname[r]:=0;{:67};opentfmfile;
...
'---not loaded, bad design size (',d:1,')!')else if inTFM(q)then{64:}
...
end{:64};end;if outmode=0 then writeln(stdout,' ');
if tfmfile then xfclose(tfmfile,curname);free(curname);
end{:63}else{61:}begin if fontchecksum[f]<>c then writeln(stdout,
...
'---font name doesn''t match previous definition!');end{:61};end;
{:60}{70:}procedure flushtext;var k:0..linelength;
...
{:76}{77:}function rulepixels(x:integer):integer;var n:integer;
begin n:=trunc(conv*x);
if n<conv*x then rulepixels:=n+1 else rulepixels:=n;end;
{:77}{80:}{83:}function specialcases(o:eightbits;p,a:integer):boolean;
label 46,44,30,9998;var q:integer;k:integer;badchar:boolean;
pure:boolean;vvv:integer;begin pure:=true;
case o of{86:}157,158,159,160:begin if abs(p)>=5*fontspace[curfont]then
...
{:86}{87:}171,172,173,174,175,176,177,178,179,180,181,182,183,184,185,
...
goto 30;end{:88};247:begin if not showing then begin flushtext;
...
44:{93:}if(v>0)and(p>0)then if v>2147483647-p then begin if not showing
...
);printfont(curfont);end;goto 30{:95};9998:pure:=false;
30:specialcases:=pure;end;{:83}function dopage:boolean;
...
hh:=0;vv:=0;while true do{81:}begin a:=curloc;showing:=false;o:=getbyte;
p:=firstpar(o);
if eof(dvifile)then begin writeln(stderr,'Bad DVI file: ',
'the file ended prematurely','!');uexit(1);end;
{82:}if o<128 then{89:}begin if(o>32)and(o<=126)then begin outtext(p);
...
end{:89}else case o of 128,129,130,131:begin if outmode>0 then begin
...
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;goto 42;
end;{84:}138:begin if outmode>1 then begin showing:=true;
...
{:84}{85:}143,144,145,146:begin if(p>=fontspace[curfont])or(p<=-4*
...
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;q:=p;
goto 43;end;
{:85}others:if specialcases(o,p,a)then goto 30 else goto 9998 end{:82};
41:{90:}if(p<fontbc[curfont])or(p>fontec[curfont])then q:=2147483647
...
hh:=hh+rulepixels(q);goto 43{:91};
43:{92:}if(h>0)and(q>0)then if h>2147483647-q then begin if not showing
then begin flushtext;showing:=true;
...
maxhsofar:=abs(h);end;goto 30{:92};
45:{94:}if showing then if outmode>2 then begin writeln(stdout,' ');
write(stdout,'level ',ss:1,':(h=',h:1,',v=',v:1,',w=',w:1,',x=',x:1,
',y=',y:1,',z=',z:1,',hh=',hh:1,',vv=',vv:1,')');end;goto 30{:94};
30:if showing then writeln(stdout,' ');end{:81};
9998:writeln(stdout,'!');dopage:=false;9999:end;
{:80}{96:}{100:}procedure scanbop;var k:0..255;
...
{:100}procedure skippages(bopseen:boolean);label 9999;var p:integer;
...
{97:}repeat if eof(dvifile)then begin writeln(stderr,'Bad DVI file: ',
'the file ended prematurely','!');uexit(1);end;k:=getbyte;
...
end;others:end;until k=140;{:97};bopseen:=false;end;9999:end;
{:96}{104:}procedure readpostamble;var k:integer;p,q,m:integer;
...
if outmode<4 then{105:}begin if maxv+99<maxvsofar then writeln(stdout,
...
pagecount:1,' pages, not ',totalpages:1,'!');end{:105};
{107:}repeat k:=getbyte;if(k>=243)and(k<247)then begin p:=firstpar(k);
definefont(p);writeln(stdout,' ');k:=138;end;until k<>138;
if k<>249 then writeln(stdout,'byte ',curloc-1:1,' is not postpost!')
{:107};{106:}q:=signedquad;
...
'not enough signature bytes at end of file (',curloc-k:1,')');{:106};
end;{:104}{108:}begin initialize;
{57:}writeln(stdout,'Options selected:');
...
printreal(newmag/1000.0,8,3);writeln(stdout,'')end{:57};
{110:}opendvifile;p:=getbyte;
...
'!');{111:}numerator:=signedquad;denominator:=signedquad;
...
writeln(stdout,' pixels per DVI unit'){:111};p:=getbyte;
write(stdout,'''');while p>0 do begin p:=p-1;
write(stdout,xchr[getbyte]);end;writeln(stdout,'''');
afterpre:=curloc{:110};if outmode=4 then begin{101:}n:=dvilength;
...
firstbackpointer:=signedquad{:101};inpostamble:=true;readpostamble;
inpostamble:=false;{103:}q:=postloc;p:=firstbackpointer;startloc:=-1;
...
if pagecount<>totalpages then writeln(stdout,'there are really ',
pagecount:1,' pages, not ',totalpages:1,'!'){:103};end;skippages(false);
if not inpostamble then{112:}begin while maxpages>0 do begin maxpages:=
...
if inpostamble then goto 30;end;30:end{:112};
