...
#endif
{
  ...
  packjobname ( 742 ) ;
  while ( ! wopenout ( basefile ) ) promptfilename ( 1073 , 742 ) ;
  printnl ( 1074 ) ;
...
#endif
{
  ...
  baseengine [x - 1 ]= 0 ;
  if ( strcmp ( enginename , baseengine ) ) 
  {
    ;
    fprintf ( stdout , "%s%s%s%ld\n",  "---! " , stringcast ( nameoffile + 1 ) ,     " was written by " , (long)baseengine ) ;
    libcfree ( baseengine ) ;
    goto lab6666 ;
  } 
  libcfree ( baseengine ) ;
...
void mainbody() {
  ...
  termoffset = 0 ;
  fileoffset = 0 ;
  Fputs ( stdout ,  "This is METAFONT, Version 2.718281" ) ;
