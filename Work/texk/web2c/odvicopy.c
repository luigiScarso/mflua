...
#endif
{
  ...
  parsearguments () ;
  Fputs ( termout ,  "This is ODVIcopy, Version 1.5" ) ;
  fprintf ( termout , "%s\n",  versionstring ) ;
...
#endif
{
  ...
  else Result = ( a - intcast ( 256 ) ) * intcast ( 256 ) + b ;
  return Result ;
} 
int16u 
#ifdef HAVE_PROTOTYPES
...
#endif
{
  ...
  if ( a < 128 ) 
  Result = ( a * intcast ( 256 ) + b ) * intcast ( 256 ) + c ;
  else Result = ( ( a - intcast ( 256 ) ) * intcast ( 256 ) + b ) * intcast ( 
  256 ) + c ;
  return Result ;
} 
int24u 
...
#endif
{
  ...
  if ( a < 128 ) 
  Result = ( ( a * intcast ( 256 ) + b ) * intcast ( 256 ) + c ) * intcast ( 
  256 ) + d ;
  else Result = ( ( ( a - intcast ( 256 ) ) * intcast ( 256 ) + b ) * intcast 
  ( 256 ) + c ) * intcast ( 256 ) + d ;
  return Result ;
...
#endif
{
  ...
  if ( tfmb0 > 127 ) 
  badfont () ;
  else firsttwo = tfmb0 * intcast ( 256 ) + tfmb1 ;
  if ( ( firsttwo != 0 ) ) 
  {
    ofmlevel = -1 ;
    if ( tfmb2 > 127 ) 
    badfont () ;
    else lh = tfmb2 * intcast ( 256 ) + tfmb3 ;
    readtfmword () ;
    if ( tfmb0 > 127 ) 
    badfont () ;
    else bc = tfmb0 * intcast ( 256 ) + tfmb1 ;
    if ( tfmb2 > 127 ) 
    badfont () ;
    else ec = tfmb2 * intcast ( 256 ) + tfmb3 ;
    if ( ec < bc ) 
    {
      bc = 1 ;
      ec = 0 ;
    ...
    if ( tfmb0 > 127 ) 
    badfont () ;
    else nw = tfmb0 * intcast ( 256 ) + tfmb1 ;
    if ( ( nw == 0 ) || ( nw > 256 ) ) 
    badfont () ;
	...
	{
	  if ( tfmb0 < 128 ) 
	  w = ( ( tfmb0 * intcast ( 256 ) + tfmb1 ) * intcast ( 256 ) + tfmb2 
	  ) * intcast ( 256 ) + tfmb3 ;
	  else w = ( ( ( tfmb0 - intcast ( 256 ) ) * intcast ( 256 ) + tfmb1 ) 
	  * intcast ( 256 ) + tfmb2 ) * intcast ( 256 ) + tfmb3 ;
	...
	else if ( l == 2 ) 
	{
	  if ( tfmb0 > 127 ) 
	  badfont () ;
	  checkdesignsize ( round ( tfmconv * ( ( ( tfmb0 * intcast ( 256 ) + 
	  tfmb1 ) * intcast ( 256 ) + tfmb2 ) * intcast ( 256 ) + tfmb3 ) ) ) 
	  ;
	} 
      } 
    while ( l++ < for_end ) ;} 
    ...
    if ( tfmb2 > 127 ) 
    badfont () ;
    else ofmlevel = tfmb2 * intcast ( 256 ) + tfmb3 ;
    readtfmword () ;
    if ( tfmb0 > 127 ) 
    badfont () ;
    ...
    {
      readtfmword () ;
      nco = ( ( ( tfmb0 * 256 + tfmb1 ) * 256 + tfmb2 ) * 256 + tfmb3 ) / 2 ;
      readtfmword () ;
      readtfmword () ;
  ...
  if ( ofmlevel <= 0 ) 
  {
	...
	if ( tfmb0 > 127 ) 
	badfont () ;
	else tfmwidth = tfmb0 * intcast ( 256 ) + tfmb1 ;
      } 
    } 
...
#endif
{
  register integer Result; xfseek ( dvifile , 0 , 2 , "odvicopy" ) ;
  dviloc = xftell ( dvifile , "odvicopy" ) ;
  Result = dviloc ;
  return Result ;
} 
void 
